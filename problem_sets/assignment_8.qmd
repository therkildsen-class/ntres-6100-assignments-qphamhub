...
Answer: The functions are similar across every line/code chunk of mine. The data input and the objects that I assign those data to are different.
...
-   **Vectorization is preferable for basic, simple operations whereas loops are for iteration of data**
...
-   I used AI by providing instructions and examples and output (chain-of-thought) to fix my problem of the mutate function in Exercise 2.4 - my code plots year in the format of 00-XX (e.g. 0088), instead of 19-XX (e.g. 1988).

-   AI suggested that i need to convert year data from 2-digit to 4-digit to be interpreted in R and suggested that I added extra lines within the mutate function:

-   Before: df_24 \<- df_24 \|\> mutate(year_month = as.Date(paste(year, month, "15", sep = "-")))

-   After: df_24 \<- df_24 \|\> mutate(year = as.integer(year), year = ifelse(year \< 100, year + 1900, year), year_month = as.Date(paste(year, month, "15", sep = "-"))
